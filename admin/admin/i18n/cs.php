...
define('INVALID_CHARS_ERROR', 'Následující %target_name% obsahuje nestandardní neplatné znaky:<br>%target_values%<br><br>Vy <strong>MUST</strong> změňte tato jména, nebo bude blokována PHPCG funkcionalita.<br><br>Povolené znaky jsou: malá, velká, čísla a podtržítka. Čísla nejsou povolena jako 1. znak.<br><br><strong>Po dokončení klikněte na tlačítko „znovu načíst strukturu databáze“.</strong>.');
...
define('MISSING_TABLE_IN_AUTHENTIFICATION_MODULE', 'Tabulka <strong>%TABLE%</strong> se používá v panelu administrátora, ale nebyla nainstalována s autentizačním modulem. <br> Aby bylo možné přidat modul ověřování, musí být znovu nainstalována tabulka <strong>%TABLE%</strong> autentizačního modulu.');
...
define('REMOVE_ADMIN_AUTHENTIFICATION_MODULE_HELPER', '<p>Tím vymažete všechny generátory &amp; Admin uživatele &amp; soubory profilů</p><p>2 databázové tabulky a jejich data <strong>NEBUDOU</strong> smazány - pokud je chcete odstranit, ručně</p><p>Pokud modul přeinstalujete později a tyto dvě tabulky již existují,  <strong>NEBUDOU</strong> přepsány.</p>');
...
define('SIDE_BY_SIDE_COMPARISON_HELPER', 'Poté klikněte na části, které chcete ponechat v levém a pravém sloupci <strong>"' . MERGE . '"</strong> uložit vaše volby');
...
define('TIME_PLACEHOLDER', 'Zástupný symbol času');
...
define('USER_MANAGEMENT_SUCCESSFULLY_CREATED', 'Správa uživatelů byla úspěšně vytvořena. Nyní můžete přidávat / upravovat uživatelské &amp; profily.<br><strong>Formuláře pro mazání uživatelských a uživatelských profilů musí být vytvořeny pomocí generátoru JHUcto.</strong>');
...
$filter_help .= '</dl>' . "\n";
$filter_help .= '<p><strong>Jako příklad bude uveden dotaz:</strong></p><pre class=" mb-20"><code>SELECT DISTINCT authors.name, authors.first_name, articles.authors_id FROM articles INNER JOIN authors ON articles.authors_id = authors.id</code></pre>' . "\n";
$filter_help .= '<p><strong>Když si uživatel vybral položku ze seznamu:</strong></p><pre><code>SELECT DISTINCT authors.name, authors.first_name, articles.authors_id FROM articles INNER JOIN authors ON articles.authors_id = authors.id WHERE articles.authors_id = [posted value]</code></pre>' . "\n";
$filter_help .= '</div>' . "\n";
...
define('RESET_DATA_CHOICES_HELP_2', '<p><span class="text-red-300 prepend">*</span>Pokud se struktura tabulky změnila, bude aktualizována (pole přidat / změnit / odstranit). Existující data budou <strong>zachována</strong> (vlastní názvy polí, filtry, typy polí, výběr hodnot, ...)</p>');
define('RESET_DATA_CHOICES_HELP_3', '<p><span class="text-red-400 prepend">**</span>Pokud se struktura tabulky změnila, bude aktualizována (pole přidat / změnit / odstranit). Existující data budou <strong>resetována</strong> (vlastní názvy polí, filtry, typy polí, výběr hodnot, ...)</p>');

define('ALLOW_CRUD_IN_LIST', 'Povolit uživatelům přidávat / upravovat / mazat záznamy z internetu <strong>READ LIST</strong>');

define('EXPLAIN_RELATION', 'Vysvětlete tento vztah');
$relation_many_to_many = '<div class="well w-100 p-5 bg-slate-100">';
$relation_many_to_many .= '<h5><span class="badge badge-info">MANY_TO_MANY vztah</span></h5><p>Povolení tohoto vztahu vám umožní spojit jeden nebo více <code class="badge badge-secondary">%target_table%</code> s každým záznamem tabulky <code class="badge badge-secondary">%origin_table%</code>.</p><p class="mb-5"> Tabulka <code class="badge badge-secondary">%intermediate_table%</code> je v tomto případě <strong>čistá relační tabulka</strong> která spojuje <code class="badge badge-secondary">%target_table%</code> s <code class="badge badge-secondary">%origin_table%</code> tabulkou.</p><p>The <strong>READ LIST</strong> zobrazí záznamy v tabulce <code class="badge badge-secondary">%target_table%</code> ve vnořené tabulce.</p><p>The <strong>CREATE</strong> a <strong>EDIT</strong> formuláře umožní přiřadit záznamy tabulky <code class="badge badge-secondary">%target_table%</code> k <code class="badge badge-secondary">%origin_table%</code> přidání/editaci.</p>';
...
$relation_one_to_many .= '<h5><span class="badge badge-info">ONE_TO_MANY vztah</span></h5><p class="mb-5">Pokud povolíte tento vztah, <strong>READ LIST</strong> zobrazí se pro každý záznam v tabulce <code class="badge badge-secondary">%target_table%</code> odpovídající záznamy v tabulce <code class="badge badge-secondary">%original_table%</code> v rozevírací tabulce.</p><p>Pokud povolíte <strong><em>' . ALLOW_CRUD_IN_LIST . '</em></strong> možnost, kterou budou uživatelé moci přidat / upravit / odstranit záznamy z tabulky <code class="badge badge-secondary">%origin_table%</code> přímo z této tabulky .</p>';
...
<p><strong>Váš navigační panel používá jednu nebo více tabulek, jejichž strukturu je třeba regenerovat.</strong></p>
<ol>
...
$users_profiles_helper .= '<li>Nastavit <strong><em>Čtení</em></strong>, <strong><em>Aktualizace</em></strong>, <strong><em>Vytvoření/Smazání</em></strong> práva tabulky, která chcete omezit "<strong>Restricted</strong>" v rozevíracím seznamu</li>';
$users_profiles_helper .= '<li>V <strong><em>Omezovacím dotazu</em></strong> pole, vložte <strong><em>WHERE</em></strong> dotaz k omezení uživatelských oprávnění </li>';
$users_profiles_helper .= '</ol>';
...
$users_profiles_helper .= '<hr>';
$users_profiles_helper .= '<p>Vaše tabulka <strong>MUST</strong> má <strong>přímý nebo nepřímý</strong> vztah k <code class="font-weight-bold">' . $ut . '</code>. Příklad:</p>';
