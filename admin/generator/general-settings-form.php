...
if ($_SERVER["REQUEST_METHOD"] == "POST" && Form::testToken('general-settings-form') === true) {
    ...
    if ($validator->hasErrors()) {
        ...
        $userConf->collapse_inactive_sidebar_categories = $_POST['collapse_inactive_sidebar_categories'];
...
$form->addCheckbox('collapse_inactive_sidebar_categories', '', 1, 'class=lcswitch, data-ontext=Yes, data-offtext=No, data-theme=blue');
...
$form->addBtn('button', 'reset-skin', 1, '<i class="' . ICON_RESET . ' prepend"></i>' . RESET . ' ' . ADMIN_SKIN_TXT, 'class=btn btn-sm btn-warning float-right');
...
<body>
            ...
            <div class="col col-md-10 my-5">
                ...
                <div class="card card-default">
                    <div class="card-header">
                        <h4 class="card-title text-semibold mb-0"><?php echo GENERAL_SETTINGS; ?></h4>
                    </div>
                    <div class="card-body">
                        <?php
                        if (DEMO === true) {
                        ...
                        <div class="alert alert-info has-icon">
                            <h4 class="mb-0">The <em>General Settings</em> form is disabled in this demo.</h4>
                        </div>
                            <?php
                        }
                        ?>
    ...
    loadjs.ready('core', function() {
        $('button[name="reset-skin"]').on('click', function() {
            $('input[name="default_header_class"]').val('');
            $('input[name="default_sidebar_user_class"]').val('');
            $('input[name="default_body_class"]').val(<?php echo $userConf->default_body_class; ?>);
            $('input[name="default_card_class"]').val(<?php echo $userConf->default_card_class; ?>);
            $('input[name="default_card_heading_class"]').val(<?php echo $userConf->default_card_heading_class; ?>);
            $('input[name="default_card_footer_class"]').val(<?php echo $userConf->default_card_footer_class; ?>);
            $('input[name="default_table_heading_background"]').val(<?php echo $userConf->default_table_heading_background; ?>);
            $('input[name="default_buttons_background"]').val(<?php echo $userConf->default_buttons_background; ?>);
        });
    });
    </script>
    <?php
