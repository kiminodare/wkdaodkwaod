...
$legend_attr       = 'class=bg-slate-600 text-light';
$legend_icon_color = 'text-white';
...
if (!empty($generator->table)) {
    ...
    // START 1st card
    $form_select_fields->startCard(SELECT_ACTION);
    ...
    $form_select_fields->addHtml('<div class="card-deck mb-3">
        <a href="#" class="choose-action-radio card bg-dark" id="build_read">
            <div class="card-body d-flex flex-column justify-content-center">
                <h6 class="card-title text-center text-white my-4"><span class="rounded-circle bg-pink-400"><i class="' . ICON_CHECKMARK . '"></i></span>' . BUILD . ' Read List</h6>
            </div>
        </a>
        <a href="#" class="choose-action-radio card bg-dark" id="build_create_edit">
            <div class="card-body d-flex flex-column justify-content-center">
                <h6 class="card-title text-center text-white my-4"><span class="rounded-circle bg-pink-400"><i class="' . ICON_CHECKMARK . '"></i></span>' . BUILD . ' Create / Update Forms</h6>
            </div>
        </a>
        <a href="#" class="choose-action-radio card bg-dark" id="build_delete">
            <div class="card-body d-flex flex-column justify-content-center">
                <h6 class="card-title text-center text-white my-4"><span class="rounded-circle bg-pink-400"><i class="' . ICON_CHECKMARK . '"></i></span>' . BUILD . ' Delete Form</h6>
            </div>
        </a>
    </div>');
    ...
    $form_select_fields->startCard(SELECT_OPTIONS_FOR_PAGINATED_LIST);
    ...
    $form_select_fields->addHtml('<div class="' . $row_last_child_class . ' justify-content-end">'); // START row
    $form_select_fields->addHtml('<div class="col-md-8 pt-3 pb-3"><label>' . FILTER_DROPDOWNS . '</label></div>');

    $form_select_fields->addHtml('<div class="col-md-4 pt-3 pb-3 text-right"><button type="button" data-toggle="collapse" data-target="#filter-help" aria-expanded="false" aria-controls="filter-help" class="btn btn-sm bg-slate dropdown-toggle dropdown-light">' . NEED_HELP . ' ?</button></div>');
    ...
    for ($i = 0; $i < count($generator->list_options['filters']); $i++) {
        $form_select_fields->addFilterFields($generator->table, $i);
    }
    ...
    if (count($generator->external_columns) > 0) {
        ...
        foreach ($generator->external_columns as $key => $ext_col) {
            ...
            $form_select_fields->addBtn('button', 'rs_ext_col_helper_btn_' . $i, 1, EXPLAIN_RELATION, 'data-toggle=collapse, data-target=#rs_ext_col_helper_' . $i . ', aria-expanded=false, aria-controls=rs_ext_col_helper_' . $i . ', class=btn btn-sm bg-slate mt-2 mt-md-0 dropdown-toggle dropdown-light');
    ...
    $form_select_fields->addHtml('<div class="slide-div">');
    $form_select_fields->addHtml('  <div class="card card-default">');
    $form_select_fields->addHtml('      <div class="card-header">' . SELECT_FIELDS_TYPES_FOR_CREATE_UPDATE . '</div>');
    ...
    $form_select_fields->startCard(SELECT_OPTIONS_FOR_SINGLE_ELEMENT_LIST);
    ...
    if (count($generator->external_columns) > 0) {
        ...
        foreach ($generator->external_columns as $key => $ext_col) {
            ...
            $form_select_fields->addBtn('button', 'rs_ext_col_helper_btn_' . $i, 1, EXPLAIN_RELATION, 'data-toggle=collapse, data-target=#rs_ext_col_helper2_' . $i . ', aria-expanded=false, aria-controls=rs_ext_col_helper2_' . $i . ', class=btn btn-sm bg-slate dropdown-toggle dropdown-light');
    ...
    $form_select_fields->addHtml('<div class="slide-div">');
    $form_select_fields->addHtml('<div class="card card-default">');
    $form_select_fields->addHtml('<div class="card-header">' . SELECT_OPTIONS_FOR_DELETE_FORM . '</div>');
    $form_select_fields->addHtml('<div class="card-body">');
...
<body<?php echo $body_padding_style; ?>>
    ...
    <nav class="navbar navbar-expand-md navbar-dark bg-indigo navbar-tall">
        <h4 class="ml-2 mb-0"><i class="fas fa-wrench fa-lg position-left"></i>PHP CRUD Generator</h4>
    ...
    <div class="container">
        ...
        <div class="row justify-content-md-center">
            <div class="col-md-10">
                <div class="alert alert-info has-icon shadow-sm">
                    ...
                    <p><strong>You are here in the PHP CRUD generator, which allows you to generate your <a href="https://www.phpcrudgenerator.com/admin/home">Bootstrap admin panel</a> from any MySQL database in a few clicks.</strong></p>
                    <hr>
                    <ol>
                        <li class="mb-2"><strong>Select the table that you want to add in your admin panel.</strong></li>
                        <li class="mb-2"><strong>To create your CRUD pages, click one of the 3 big buttons:</strong>
                        ...
                        <li><strong>Choose your options for each field then confirm at the bottom of the page</strong></li>
                    </ol>
        ...
        <div class="row justify-content-md-center">
            <div class="col-md-10 mb-4">
                <div class="card card-default">
                    <div class="card-header"><?php echo TOOLS; ?></div>
                    <div class="card-body">
        ...
        <div class="row justify-content-md-center">
            <div class="col-md-10 mb-4">
                <div class="card card-default card-collapsed">
                    <div class="card-header"><?php echo $generator->database . ' ' . RELATIONS; ?>
                        <div class="heading-elements">
                            <a class="dropdown-toggle" data-toggle="collapse" href="#relations" role="button" aria-expanded="false" aria-controls="relations"></a>
                        </div>
                    </div>
        ...
        <div class="row justify-content-md-center">
            <div class="col-md-10 mb-4">
                <div class="card card-default">
                    <div class="card-header"><?php echo CHOOSE_TABLE; ?></div>
                    <div class="card-body">
    ...
    <?php
    if (isset($form_select_fields)) {
        $form_select_table->printJsCode('core');
    }
    if (isset($form_select_fields)) {
        $form_select_fields->printJsCode('core');
    }
    if (!empty($generator->diff_files_form)) {
        $generator->diff_files_form->printJsCode('core');
    }
    ?>
    </body>
